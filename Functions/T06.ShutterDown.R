...
AF.Performance.Summary <- function()
{
  ...
  x <- IB.04.activity %>%
        group_by(ticker, algoId, Type, DP.Method, MA.Type, Period, IB.action) %>%
        arrange(ticker, algoId, Type, DP.Method, MA.Type, Period, IB.action, order.ts) %>%
        mutate(ds = min(as.Date(order.ts))) %>%
        group_by(ticker, algoId, Type, DP.Method, MA.Type, Period, IB.action, ds) %>%
        summarise(price = weighted.mean(price, units, na.rm = TRUE),
                  units = sum(units, na.rm = TRUE) ) %>%
        ungroup() %>%
        data.table::setDT() %>%
        data.table::dcast(ticker + algoId + Type + DP.Method + MA.Type + Period ~ IB.action,
